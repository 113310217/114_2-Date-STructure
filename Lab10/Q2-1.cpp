...
#include <ctime>  // 引入時間標頭檔用以初始化隨機種子
using namespace std;
...
public:
 ...
 void heapify(int i) { // 堆化函式（確保以 i 為根的子樹符合 Max Heap 性質）
  ...
  // heap[left] > heap[largest]，比較左子節點的值是否大於目前記錄的最大值(父節點) 
  // 若兩者皆成立，代表左子節點大於父節點，違反 Max Heap 性質 
  if (left < heap.size() && heap[left] > heap[largest]) { //  
   largest = left; // 將最大值指標 largest 轉向左子節點的索引位置
  }
  ...
  // heap[right] > heap[largest]，比較右子節點的值是否比目前的最大值位置還要大 
  // 若兩者皆成立，代表右子節點是三者之中的最大值 
  if (right < heap.size() && heap[right] > heap[largest]) {
   largest = right; // 將最大值指標 largest 更新為右子節點的索引位置 
  }

  // if判斷式，如果最大的不是父節點，交換並繼續堆化                   
  // 遞迴處理受影響的子樹
  // 第三步：若最大值不是父節點，執行交換與遞迴調整
  // 若 largest 不等於 i，則代表經過上面兩個 if 的比較後，左子或右子節點的值比父節點還要大
  if (largest != i) {
   swap(heap[i], heap[largest]); // 為了維持 Max Heap 原則，將父節點的值與最大子節點的值進行實體位置對調
   heapify(largest);             // 因為原本較小的父節點被換下去，可能會導致該位置以下的子樹結構被破壞
                                // 因此，必須以剛才被換下去的新位置（即 largest 索引）作為新的根節點
							    //再次遞迴呼叫 heapify，像漣漪一樣持續向下修正，直到整棵子樹都安全地符合 Max Heap 為止
  }
 }
 ...
 void printHeap() {
  ...
  while (!heap.empty()) {
   cout << heap[0] << " ";  // 列印出樹頂 heap[0] (當前最大值)
